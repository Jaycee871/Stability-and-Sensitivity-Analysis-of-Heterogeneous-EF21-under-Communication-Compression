...
class FullHeterogeneityConfig:
    ...
    The arithmetic means L_bar and mu_bar are held fixed while the ratios
    tau_L=L2/L1 and tau_mu=mu2/mu1 vary independently. This preserves the
    average condition number kappa_bar=L_bar/mu_bar and isolates regularity
    heterogeneity from changes in average problem scale.
    """
...
def empirical_eta_star_full(config: FullHeterogeneityConfig) -> float:
    """Inherited Empirical Law 4.3 step size on the controlled full path."""
    config.validate()
...
def run_full_grid(
    ...
    *,
    ...
    Returns (valid_rows, invalid_count). Invalid cells arise when a requested
    ratio pair violates the inherited regularity requirement mu_i <= L_i.
    """
